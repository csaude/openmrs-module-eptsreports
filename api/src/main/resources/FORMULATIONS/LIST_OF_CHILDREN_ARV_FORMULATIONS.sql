			...
			max_filaF.D3, 
			max_filaF.D4, 
			DATE_FORMAT(date(max_filaF.proxima_levantamento), '%%d/%%m/%%Y') as proxima_levantamento,
			DATE_FORMAT(date(max_consulta_final.data_consulta), '%%d/%%m/%%Y') as data_consulta,
			max_consulta_final.peso, 
			max_consulta_final.abordagem,
			max_consulta_final.dispensa, 
			DATE_FORMAT(date(max_consulta_final.proxima_consulta), '%%d/%%m/%%Y') as proxima_consulta,
			max_consulta_final.codeReg 
			from  
			( 
	         %s
			) coorte12meses_final  
