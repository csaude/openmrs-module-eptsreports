        ...
        (
         ...
         SELECT tx_new.patient_id,tx_new.art_start_date
           FROM 
              ...
              AND e.encounter_datetime <=date_sub(date(concat(2024,'-06','-20')), interval 24 MONTH)
              ...
              AND o.value_datetime <=date_sub(date(concat(2024,'-06','-20')), interval 24 MONTH)
              AND e.location_id=:location 
              GROUP BY p.patient_id 
              ) 
              art_start GROUP BY patient_id 
          ) tx_new WHERE art_start_date BETWEEN  date_sub(date(concat(2024,'-12','-21')), interval 36 MONTH) and  date_sub(date(concat(2024,'-06','-20')), interval 24 MONTH)
        )coorte24Meses
            left join
        (
              ...
              (
                ...
                WHERE p.voided=0 AND e.voided=0 AND e.encounter_type=53 and obsData.value_datetime<=date_sub(date(concat(2024,'-06','-20')), interval 24 MONTH)
                AND e.location_id=:location 
                GROUP BY p.patient_id 
                union
                ...
                WHERE pg.voided=0 
                and ps.start_date<=date_sub(date(concat(2024,'-06','-20')), interval 24 MONTH)
                and ps.voided=0 
                and p.voided=0 
                and pg.program_id=2 
        ...
        (
         SELECT tx_new.patient_id,tx_new.art_start_date
           FROM 
              ...
              art_start GROUP BY patient_id 
          ) tx_new WHERE art_start_date BETWEEN  date_sub(date(concat(2024,'-12','-21')), interval 48 MONTH) and  date_sub(date(concat(2024,'-06','-20')), interval 36 MONTH)
                      ...
                      (
                               ...
                               SELECT tx_new.patient_id,tx_new.art_start_date
                                 FROM 
                                    ...
                                    art_start GROUP BY patient_id 
                                ) tx_new --WHERE art_start_date BETWEEN  date_sub(date(concat(2024,'-12','-21')), interval 48 MONTH) and   date_sub(date(concat(2024,'-12','-21')), interval 12 month)
                            )tx_new
                            inner join
                            ( 
                            ...
                            union
                               ...
                               SELECT tx_new.patient_id,tx_new.art_start_date
                                 FROM 
                                    ...
                                    art_start GROUP BY patient_id 
                                ) tx_new --WHERE art_start_date BETWEEN  date_sub(date(concat(2024,'-12','-21')), interval 48 MONTH) and   date_sub(date(concat(2024,'-12','-21')), interval 24 month)
                            )tx_new
                            inner join
                            ( 
                            select patient_id,max(data_obito) data_obito from  
           ...
           select final.patient_id, tpt.valor1,tptFinal.valor2,resultadoCd4Inicial.valor3,resultadoDataCd4Inicial.valor4
            ...
            (
               SELECT tx_new.patient_id,tx_new.art_start_date
                     ...
                     WHERE ((tx_new.art_start_date BETWEEN  date_sub(date(concat(2024,'-12','-21')), interval 24 MONTH) and  date_sub(date(concat(2024,'-06','-20')), interval 12 MONTH)) 
                        OR (tx_new.art_start_date BETWEEN date_sub(date(concat(2024,'-12','-21')), interval 36 MONTH) and  date_sub(date(concat(2024,'-06','-20')), interval 24 MONTH))
                        OR (tx_new.art_start_date BETWEEN  date_sub(date(concat(2024,'-12','-21')), interval 48 MONTH) and  date_sub(date(concat(2024,'-06','-20')), interval 36 MONTH)))
             ) final
            left join
            (
            select 
            ...
            (
              ...
              WHERE e.voided=0 AND o.voided=0 AND p.voided=0 AND e.encounter_type in (18) 
              AND e.location_id=:location 
              AND e.encounter_datetime <=date_sub(date(concat(2024,'-06','-20')), interval 12 MONTH)
              GROUP BY p.patient_id 
              UNION 
              SELECT p.patient_id, MIN(value_datetime) art_start_date FROM 
              patient p 
              ...
              WHERE p.voided=0 AND e.voided=0 AND o.voided=0 AND e.encounter_type=52 
              AND o.concept_id=23866 AND o.value_datetime is NOT NULL 
              AND o.value_datetime <=date_sub(date(concat(2024,'-06','-20')), interval 12 MONTH)
              AND e.location_id=:location 
              GROUP BY p.patient_id 
              ) 
              art_start
