 select                             coorte12meses_final.patient_id,                               
                                    DATE_FORMAT(date(coorte12meses_final.data_inicio), '%d/%m/%Y') as art_start_date,
                                    DATE_FORMAT(date(coorte12meses_final.data_fila), '%d/%m/%Y') as ultimo_lev,
                                    DATE_FORMAT(date(coorte12meses_final.data_usar), '%d/%m/%Y') as proximo_marcado,
                                    DATE_FORMAT(date(coorte12meses_final.data_estado), '%d/%m/%Y') as data_estado,
                                    coorte12meses_final.estado,
                                    DATE_FORMAT(date(coorte12meses_final.data_fila), '%d/%m/%Y') as ultimoFila,
                                    DATE_FORMAT(date(coorte12meses_final.data_proximo_lev), '%d/%m/%Y') as proximoLevantamentoFila,
                                    DATE_FORMAT(date(coorte12meses_final.data_recepcao_levantou), '%d/%m/%Y') as ultimoLevantamentoMCard,
                                    DATE_FORMAT(date(coorte12meses_final.data_recepcao_levantou30), '%d/%m/%Y') as proximoLevantamentoMCard,
                                    ...
                                    DATE_FORMAT(date(consultaClinica.data_seguimento), '%d/%m/%Y') as ultimaConsulta,
                                    DATE_FORMAT(date(consultaClinica.value_datetime), '%d/%m/%Y') as proximaConsulta,
                                    menthorMotherFC.menthorMother,
                                    teenagerAndYoungFC.teenagerAndYoung,
                                    championManFC.championMan,
                                    DATE_FORMAT(date(max_consultaApss.data_seguimento), '%d/%m/%Y') as ultimaConsultaApss,
                                    DATE_FORMAT(date(max_consultaApss.value_datetime), '%d/%m/%Y') as proximaConsultaApss,
                                    menthorMotherAPSS.menthorMotherApss,
                                    teenagerAndYoungAPSS.teenagerAndYoungApss,
                                    championManAPSS.championManApss,
			                        ...
			                        MDC.MDC4 AS MDC4,
			                        MDC.MDC5 as MDC5,
			                        IF(ISNULL(keyPop.fcWithKeyPopDate),'N/A',DATE_FORMAT(date(keyPop.fcWithKeyPopDate), '%d/%m/%Y')) as fcWithKeyPopDate,
					                keyPop.keyPopHSH,
							        keyPop.keyPopPID,
								    keyPop.keyPopREC,
								    keyPop.keyPopTS,
								    if(patOVCEntryDate.value is null,'N/A',DATE_FORMAT(date(patOVCEntryDate.value), '%d/%m/%Y')) AS oVCEntryDate,
								    if(patOVCExitDate.value is null,'N/A',DATE_FORMAT(date(patOVCExitDate.value), '%d/%m/%Y')) AS OVCExitDate,
								    if(patOVCStatus.value is null,'N/A',patOVCStatus.value) AS OVCStatus,
								    DATE_FORMAT(date(abandonoNotificado.data_estado_abandono), '%d/%m/%Y') data_estado_abandono
					             from                              
                         (
