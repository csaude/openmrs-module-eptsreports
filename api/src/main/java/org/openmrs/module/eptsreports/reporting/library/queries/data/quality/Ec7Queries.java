  ...
  public static String getEc7CombinedQuery() {
    String query =
        " SELECT "
            ...
            + " INNER JOIN  ( "
            + "		select max_estado.patient_id, max_estado.data_transferido_para "
            + "	from( "
            + "			select pp.patient_id, ps.patient_state_id, ps.state, max(max_estado.data_estado) data_transferido_para "
            + "			from( "
            + " "
            ...
            + " where pe.voided = 0 and (transferido_para.patient_id is not null) and (Seguimento.encounter_datetime>transferido_para.data_transferido_para) "
