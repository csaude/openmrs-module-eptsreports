...
public interface PrepNewQueries {
  ...
  class QUERY {
    ...
    public static final String findClientsNewlyEnrolledInPrepByOralPrepType =
        "select prepNew.patient_id from "
            + "( "
            + " select prep_new.patient_id,prep_new.data_inicio_prep "
            + " from (	select patient_id, min(data_inicio_prep) data_inicio_prep "
            + " 		from	(	select client.patient_id, min(o.obs_datetime) data_inicio_prep "
            + " 				from patient client "
            + " 					inner join encounter e on e.patient_id = client.patient_id "
            ...
            + " prep_new "
            + "inner join person pe on pe.person_id=prep_new.patient_id "
            + "where prep_new.data_inicio_prep between :startDate and :endDate and ((pe.birthdate is not null and timestampdiff(year,pe.birthdate,:endDate) >= 15) or pe.birthdate is  null) "
            + ") prepNew "
            + "left join "
            + "( "
            + "select prep.patient_id,prep.data_consulta_oral from "
            + "( "
            + "select p.patient_id, e.encounter_datetime data_consulta_oral "
            + "		from patient p "
            ...
            + "	) prep "
            + ")prepFinal on prepNew.patient_id=prepFinal.patient_id "
            + "where prepFinal.data_consulta_oral=prepNew.data_inicio_prep ";

    public static final String findClientsNewlyEnrolledInPrepByInjectablePrepType =
        "select prepNew.patient_id from "
            + "( "
            + " select prep_new.patient_id,prep_new.data_inicio_prep "
            + " from (	select patient_id, min(data_inicio_prep) data_inicio_prep "
            ...
            + "left join "
            + "( "
            + "select prep.patient_id,prep.data_consulta_injectavel from "
            + "( "
            + "select p.patient_id, e.encounter_datetime data_consulta_injectavel "
            + "		from patient p "
            + "		inner join encounter e on e.patient_id=p.patient_id "
            + "		inner join obs o on o.encounter_id=e.encounter_id "
            + "		where e.voided=0 "
            ...
            + "	) prep "
            + ")prepFinal on prepNew.patient_id=prepFinal.patient_id "
            + "where prepFinal.data_consulta_injectavel=prepNew.data_inicio_prep ";

    public static final String findClientsNewlyEnrolledInPrepByOtherPrepType =
        "select prepNew.patient_id from "
            + "( "
            + " select prep_new.patient_id,prep_new.data_inicio_prep "
            + " from (	select patient_id, min(data_inicio_prep) data_inicio_prep "
            ...
            + "left join "
            + "( "
            + "select prep.patient_id,prep.data_consulta_other from "
            + "( "
            + "select p.patient_id, e.encounter_datetime data_consulta_other "
            + "		from patient p "
            + "		inner join encounter e on e.patient_id=p.patient_id "
            + "		inner join obs o on o.encounter_id=e.encounter_id "
            + "		where e.voided=0 "
            ...
            + "	) prep "
            + ")prepFinal on prepNew.patient_id=prepFinal.patient_id "
            + "where prepFinal.data_consulta_other=prepNew.data_inicio_prep ";
  }
}
