  ...
  public static String getBaseCohortQuery() {
    ...
    final String query =
        "	SELECT p.patient_id FROM patient p INNER JOIN encounter e ON e.patient_id=p.patient_id "
            ...
            + "	union "
            + "	SELECT pg.patient_id FROM 	patient p INNER JOIN patient_program pg ON p.patient_id=pg.patient_id "
            ...
            + "	union "
            + "	SELECT p.patient_id FROM patient p INNER JOIN encounter e ON e.patient_id=p.patient_id "
            + "	WHERE e.voided=0 AND p.voided=0 AND e.encounter_type IN (92,93) "
            + "	AND e.encounter_datetime<=:endDate AND e.location_id =:location ";

    return query;
  }

  public static String getBaseCohortQueryWithoutPrep() {
