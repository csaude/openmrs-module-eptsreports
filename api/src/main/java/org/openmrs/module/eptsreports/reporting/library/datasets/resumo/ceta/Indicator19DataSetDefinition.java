...
@Component
public class Indicator19DataSetDefinition extends BaseDataSet {
  ...
  public DataSetDefinition constructDataset() {
    ...
    dataSetDefinition.setName("Indicator 19");
    ...
    CohortDefinition indicator19 = this.resumoMensalCETACohortQueries.getPatientsIndicator19();

    dataSetDefinition.addDimension(
        "indicator19Dimension",
        EptsReportUtils.map(
            this.resumoMensalCETADimensions.getResumoMensalCETADimension(indicator19), mappings));

    final CohortIndicator rmCETAIndicator19 =
        this.eptsGeneralIndicator.getIndicator(
            "RMCETA", EptsReportUtils.map(indicator19, mappings));

    addRow(
        dataSetDefinition,
        "I19",
        "Nr de pacientes transferidos",
        EptsReportUtils.map(rmCETAIndicator19, mappings),
        resumoMensalCETADataSetDefinition.getColumns("indicator19Dimension"));
