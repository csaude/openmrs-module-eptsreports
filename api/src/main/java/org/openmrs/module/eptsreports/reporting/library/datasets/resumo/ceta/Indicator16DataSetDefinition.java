...
@Component
public class Indicator16DataSetDefinition extends BaseDataSet {
  ...
  public DataSetDefinition constructDataset() {
    ...
    dataSetDefinition.setName("Indicator 16");
    ...
    CohortDefinition indicator16 = this.resumoMensalCETACohortQueries.getPatientsIndicator16();

    dataSetDefinition.addDimension(
        "indicator16Dimension",
        EptsReportUtils.map(
            this.resumoMensalCETADimensions.getResumoMensalCETADimension(indicator16), mappings));

    final CohortIndicator rmCETAIndicator16 =
        this.eptsGeneralIndicator.getIndicator(
            "RMCETA", EptsReportUtils.map(indicator16, mappings));

    addRow(
        dataSetDefinition,
        "I16",
        "Nr de pacientes que consomem outras substâncias psicoactivas (ex. Canabis, marijuana, etc) ",
        EptsReportUtils.map(rmCETAIndicator16, mappings),
        resumoMensalCETADataSetDefinition.getColumns("indicator16Dimension"));
