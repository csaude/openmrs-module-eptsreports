...
@Component
public class Indicator21DataSetDefinition extends BaseDataSet {
  ...
  public DataSetDefinition constructDataset() {
    ...
    dataSetDefinition.setName("Indicator 21");
    ...
    CohortDefinition indicator21 = this.resumoMensalCETACohortQueries.getPatientsIndicator21();

    dataSetDefinition.addDimension(
        "indicator21Dimension",
        EptsReportUtils.map(
            this.resumoMensalCETADimensions.getResumoMensalCETADimension(indicator21), mappings));

    final CohortIndicator rmCETAIndicator21 =
        this.eptsGeneralIndicator.getIndicator(
            "RMCETA", EptsReportUtils.map(indicator21, mappings));

    addRow(
        dataSetDefinition,
        "I21",
        "Nr de Óbitos",
        EptsReportUtils.map(rmCETAIndicator21, mappings),
        resumoMensalCETADataSetDefinition.getColumns("indicator21Dimension"));
