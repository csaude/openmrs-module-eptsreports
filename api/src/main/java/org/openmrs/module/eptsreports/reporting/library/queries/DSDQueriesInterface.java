    ...
    public static String findPatientsWhoArePregnantsAndBreastFeeding(TypePTV typePTV) {
      ...
      String query =
          "select patient_id from ( select inicio_real.patient_id,gravida_real.data_gravida, lactante_real.data_parto, "
              ...
              + "            inner join patient_program pg on p.patient_id=pg.patient_id "
              + "            inner join patient_state ps on pg.patient_program_id=ps.patient_program_id "
              + "            where pg.voided=0 and ps.voided=0 and p.voided=0 and pg.program_id=8 and ps.state=27 and ps.end_date is null  and ps.start_date between (:endDate - interval 18 MONTH) and :endDate and location_id=:location  "
