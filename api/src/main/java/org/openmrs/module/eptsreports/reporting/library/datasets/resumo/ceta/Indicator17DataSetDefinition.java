...
@Component
public class Indicator17DataSetDefinition extends BaseDataSet {
  ...
  public DataSetDefinition constructDataset() {
    ...
    dataSetDefinition.setName("Indicator 17");
    ...
    CohortDefinition indicator17 = this.resumoMensalCETACohortQueries.getPatientsIndicator17();

    dataSetDefinition.addDimension(
        "indicator17Dimension",
        EptsReportUtils.map(
            this.resumoMensalCETADimensions.getResumoMensalCETADimension(indicator17), mappings));

    final CohortIndicator rmCETAIndicator17 =
        this.eptsGeneralIndicator.getIndicator(
            "RMCETA", EptsReportUtils.map(indicator17, mappings));

    addRow(
        dataSetDefinition,
        "I17",
        "Nr de pacientes que interromperam o tratamento",
        EptsReportUtils.map(rmCETAIndicator17, mappings),
        resumoMensalCETADataSetDefinition.getColumns("indicator17Dimension"));
