...
public interface EC2DuplicateQueries {
  class QUERY {
    ...
    public static String findPatiendsWithDuplicatedNID =
        " select duplicatedDemographic.patient_id, "
            ...
            + "						 DATE_FORMAT(fichaClinica.encounter_datetime, '%d/%m/%Y') AS ultima_ficha_clinica, "
            ...
            + "						 from ( "
            + " "
            + "select distinct demo.patient_id,demo.gender, demo.birthdate, demo.family_name,demo.person_id, demo.person_name_id, demo.patient_identifier_id from ( "
            + "SELECT repeated.patient_id,repeated.gender, repeated.birthdate, repeated.family_name,repeated.person_id, repeated.person_name_id, repeated.patient_identifier_id "
            + "from ( "
            + "SELECT pe.person_id, pn.person_name_id, pi.patient_identifier_id,pe.gender, pe.birthdate, pn.family_name,pi.identifier,pi.identifier_type,count(pe.gender),count(pe.birthdate),count(pn.family_name) "
            + "FROM patient p "
            ...
            + "inner join patient_identifier pi on (pi.patient_id = p.patient_id and pi.voided = 0 and pi.identifier_type =2) "
            + "left join location on (location.location_id =pi.location_id and location.retired =0) "
            + "group by  pe.gender,pe.birthdate, pn.family_name "
            + "having count(pe.gender) >=2 and count(pe.birthdate) >=2 and count(pn.family_name) >=2 "
            + ") sameDemographic "
            + "inner join ( "
            ...
            + "             group by pi.identifier  having count(*) = 1 "
            + ") nidNaoRepetido on nidNaoRepetido.identifier = sameDemographic.identifier "
            + "inner join ( "
            + "SELECT pe.gender, pe.birthdate, pn.family_name,pi.identifier,p.patient_id,pe.person_id, pn.person_name_id, pi.patient_identifier_id "
            + "FROM patient p "
            + "inner join person pe on (pe.person_id = p.patient_id and pe.voided = 0) "
            + "inner join person_name pn on (pn.person_id = p.patient_id and pn.voided = 0) "
            + "inner join patient_identifier pi on (pi.patient_id = p.patient_id and pi.voided = 0) "
            + ") repeated on repeated.gender = sameDemographic.gender and sameDemographic.birthdate = repeated.birthdate and sameDemographic.family_name = repeated.family_name "
            + "where repeated.patient_id in ( "
            + "		    						select patient_id "
            + "	from "
            + "	( "
            ...
            + "inner join person_name pn on (pn.person_id = p.patient_id and pn.voided = 0) "
            + "inner join patient_identifier pi on (pi.patient_id = p.patient_id and pi.voided = 0) "
            + ") demo on demografic.gender = demo.gender and demografic.birthdate = demo.birthdate and demografic.family_name = demo.family_name "
            + " "
            + ") duplicatedDemographic "
            + "			inner join patient_identifier on (patient_identifier.patient_identifier_id = duplicatedDemographic.patient_identifier_id and patient_identifier.preferred = true and patient_identifier.identifier_type = 2) "
            + "                            inner join patient on patient.patient_id = duplicatedDemographic.patient_id "
            + "                            inner join person  on person.person_id = duplicatedDemographic.person_id "
            + "                            inner join person_name on person_name.person_name_id = duplicatedDemographic.person_name_id "
            + " "
            + " "
            ...
            + "					 AND pg.program_id = 2 "
            + "					 AND pg.location_id IN (400) "
            + "					 GROUP BY pg.patient_id "
            + "					 ) AS programState  ON duplicatedDemographic.patient_id = programState.patient_id "
            + " "
            + " left join "
