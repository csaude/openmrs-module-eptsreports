...
public class TxCurrCohortQueries {
	...
	public CohortDefinition getTxCurrCompositionCohort(final String cohortName, final boolean currentSpec) {
		...
		final CohortDefinition inARTProgramAtEndDate = this.genericCohortQueries.createInProgram("InARTProgram",
		...
		txCurrComposition.getSearches().put("2", EptsReportUtils.map(
				this.getPatientWithSTARTDRUGSObsBeforeOrOnEndDate(), "onOrBefore=${onOrBefore},location=${location}"));
		txCurrComposition.getSearches().put("3",
				EptsReportUtils.map(this.hivCohortQueries.getPatientWithHistoricalDrugStartDateObsBeforeOrOnEndDate(),
						"onOrBefore=${onOrBefore},location=${location}"));
		txCurrComposition.getSearches().put("4",
				EptsReportUtils.map(this.getPatientWithFirstDrugPickupEncounterBeforeOrOnEndDate(),
						"onOrBefore=${onOrBefore},location=${location}"));
		txCurrComposition.getSearches().put("5", EptsReportUtils.map(
				this.getPatientsWhoLeftARTProgramBeforeOrOnEndDate(), "onOrBefore=${onOrBefore},location=${location}"));
		txCurrComposition.getSearches().put("6", EptsReportUtils.map(this.getPatientsThatMissedNexPickup(),
				String.format("onOrBefore=${onOrBefore},location=${location},abandonmentDays=%s", abandonmentDays)));
		txCurrComposition.getSearches().put("7", EptsReportUtils.map(this.getPatientsThatDidNotMissNextConsultation(),
				String.format("onOrBefore=${onOrBefore},location=${location},abandonmentDays=%s", abandonmentDays)));
		txCurrComposition.getSearches().put("8", EptsReportUtils.map(
				this.getPatientsReportedAsAbandonmentButStillInPeriod(),
				String.format("onOrBefore=${onOrBefore},location=${location},abandonmentDays=%s", abandonmentDays)));
		txCurrComposition.getSearches().put("11", EptsReportUtils.map(this.getPatientsWithNextPickupDate(),
				"onOrBefore=${onOrBefore},location=${location}"));
		txCurrComposition.getSearches().put("12", EptsReportUtils.map(this.getPatientsWithNextConsultationDate(),
				"onOrBefore=${onOrBefore},location=${location}"));
	...
	private CohortDefinition getPatientWithFirstDrugPickupEncounterBeforeOrOnEndDate() {
		...
		final String query = "SELECT p.patient_id " + "FROM patient p "
				+ "INNER JOIN encounter e ON p.patient_id=e.patient_id " + "WHERE p.voided=0 AND e.encounter_type=%d "
		...
		patientWithFirstDrugPickupEncounter
				.setQuery(String.format(query, this.hivMetadata.getARVPharmaciaEncounterType().getEncounterTypeId()));
	...
	/**
	 * @return Cohort of patients registered as START DRUGS (answer to question 1255
	 *         = ARV PLAN is 1256 = START DRUGS) in the first drug pickup (encounter
	 *         type 18=S.TARV: FARMACIA) or follow up consultation for adults and
	 *         children (encounter types 6=S.TARV: ADULTO SEGUIMENTO and 9=S.TARV:
	...
	private CohortDefinition getPatientWithSTARTDRUGSObsBeforeOrOnEndDate() {
		...
		final String query = "SELECT p.patient_id FROM patient p INNER JOIN encounter e ON p.patient_id=e.patient_id "
	...
	private CohortDefinition getPatientsWhoLeftARTProgramBeforeOrOnEndDate() {
		...
		final String leftARTProgramQueryString = "select p.patient_id from patient p inner join patient_program pg on p.patient_id=pg.patient_id "
		...
		final String abandonStates = StringUtils.join(Arrays.asList(
				this.hivMetadata.getTransferredOutToAnotherHealthFacilityWorkflowState().getProgramWorkflowStateId(),
		...
		leftARTProgramBeforeOrOnEndDate.setQuery(String.format(leftARTProgramQueryString,
				this.hivMetadata.getARTProgram().getProgramId(), abandonStates));
	...
	private CohortDefinition getPatientsThatMissedNexPickup() {
		...
		final String query = "SELECT patient_id FROM (SELECT p.patient_id,max(encounter_datetime) encounter_datetime FROM patient p INNER JOIN encounter e on e.patient_id=p.patient_id WHERE p.voided=0 AND e.voided=0 AND e.encounter_type=%s"
		...
		definition.setQuery(String.format(query, this.hivMetadata.getARVPharmaciaEncounterType().getEncounterTypeId(),
	...
	private CohortDefinition getPatientsThatDidNotMissNextConsultation() {
		...
		final String query = "SELECT patient_id FROM "
				...
				+ "AND o.location_id=:location AND DATEDIFF(:onOrBefore,o.value_datetime)<:abandonmentDays";
		definition
				.setQuery(String.format(query, this.hivMetadata.getAdultoSeguimentoEncounterType().getEncounterTypeId(),
						this.hivMetadata.getARVPediatriaSeguimentoEncounterType().getEncounterTypeId(),
	...
	private CohortDefinition getPatientsReportedAsAbandonmentButStillInPeriod() {
		...
		final String query = "SELECT abandono.patient_id FROM (SELECT pg.patient_id FROM patient p INNER JOIN patient_program pg ON p.patient_id=pg.patient_id INNER JOIN patient_state ps ON pg.patient_program_id=ps.patient_program_id WHERE pg.voided=0 AND ps.voided=0 AND p.voided=0 AND pg.program_id=%d "
				+ "AND ps.state=%d "
		...
		definition.setQuery(String.format(query, this.hivMetadata.getARTProgram().getProgramId(),
				this.hivMetadata.getAbandonedWorkflowState().getProgramWorkflowStateId(),
	...
	private CohortDefinition getPatientsWithNextPickupDate() {
		...
		final String encounterTypes = StringUtils
				.join(Arrays.asList(this.hivMetadata.getARVPharmaciaEncounterType().getEncounterTypeId()), ',');
		definition.setQuery(String.format(HAS_NEXT_APPOINTMENT_QUERY,
				this.hivMetadata.getReturnVisitDateForArvDrugConcept().getConceptId(), encounterTypes));
		definition.addParameter(new Parameter("onOrBefore", "onOrBefore", Date.class));
		definition.addParameter(new Parameter("location", "location", Location.class));
		return definition;
	}
	...
	private CohortDefinition getPatientsWithNextConsultationDate() {
		...
		final String encounterTypes = StringUtils
				.join(Arrays.asList(this.hivMetadata.getAdultoSeguimentoEncounterType().getEncounterTypeId(),
						this.hivMetadata.getARVPediatriaSeguimentoEncounterType().getEncounterTypeId()), ',');
		definition.setQuery(String.format(HAS_NEXT_APPOINTMENT_QUERY,
				this.hivMetadata.getReturnVisitDateConcept().getConceptId(), encounterTypes));
		definition.addParameter(new Parameter("onOrBefore", "onOrBefore", Date.class));
		definition.addParameter(new Parameter("location", "location", Location.class));
		return definition;
	}
	...
	public CohortDefinition getTXMLPatientsWhoMissedNextApointmentCalculation() {
		FGHCalculationCohortDefinition cd = new FGHCalculationCohortDefinition(
				"txMLPatientsWhoMissedNextApointmentCalculation",
				Context.getRegisteredComponents(TxMLPatientsWhoMissedNextApointmentCalculation.class).get(0));
	...
	public CohortDefinition getPatientsMarkedDead() {
		FGHCalculationCohortDefinition cd = new FGHCalculationCohortDefinition("patientsMarkedAsDeadCalculation",
	...
	public CohortDefinitionDimension findPatientsWhoAreDeadDimension() {
		...
		dimension.addCohortDefinition("breastfeeding", EptsReportUtils.map(this.getPatientsMarkedDead(), mappings));
