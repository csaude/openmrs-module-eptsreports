  ...
  class QUERY {
    ...
    public static final String findPatientsWhoAreActiveOnArtAndInAtleastOneDSDWithViralSupression =
        "         select patient_id from ( "
            ...
            + "           select patient_id,max(data_max_consulta) data_max_consulta from ( "
            + "            select carga_viral.patient_id,e.encounter_datetime data_max_consulta, data_carga from ( "
            + "	           select ultima_carga.patient_id,max(ultima_carga.data_carga) data_carga from ( "
            + "			  select p.patient_id,max(o.obs_datetime) data_carga from patient p "
            + "					inner join encounter e on p.patient_id=e.patient_id "
            + "					inner join obs o on e.encounter_id=o.encounter_id "
            + "					where p.voided=0 and e.voided=0 and o.voided=0 and e.encounter_type in (13,6,9,53,51) and  o.concept_id in (856,1305) "
            + "					and  date(o.obs_datetime) between date_add(date_add(:endDate, interval -12 MONTH), interval 1 day) "
            + "					and :endDate and e.location_id=:location group by p.patient_id "
            + "			) ultima_carga "
            + "				inner join obs on obs.person_id=ultima_carga.patient_id "
            + "				and date(obs.obs_datetime)=date(ultima_carga.data_carga) "
            + "				where obs.voided=0 and ((obs.concept_id=856 and obs.value_numeric<1000) "
            ...
            + "			encounter e on carga_viral.patient_id=e.patient_id "
            + "			where e.voided=0 and e.encounter_type=6 and e.location_id=:location "
            + "			and e.encounter_datetime < carga_viral.data_carga "
            + "		)maxConsulta group by maxConsulta.patient_id "
            + "	) mdc inner join "
            + "		encounter e on (mdc.patient_id=e.patient_id and mdc.data_max_consulta = e.encounter_datetime) "
            ...
            + "	union "
            + "		select tipodispensa.patient_id from( "
            + "           select patient_id,max(data_max_consulta) data_max_consulta from ( "
            + "            select carga_viral.patient_id,e.encounter_datetime data_max_consulta, data_carga from ( "
            + "	           select ultima_carga.patient_id,max(ultima_carga.data_carga) data_carga from ( "
            + "			  select p.patient_id,max(o.obs_datetime) data_carga from patient p "
            ...
            + "	inner join obs o on o.encounter_id=e.encounter_id "
            + "     where e.voided=0 and tipodispensa.data_max_consulta=e.encounter_datetime and o.concept_id=23739 and o.value_coded in(23720,23888) and o.voided=0 and e.location_id=:location "
            + "     union "
            + "	select patient_id from ( "
            ...
            + "		where fila.data_levantamento<date(carga_viral.data_carga) "
    ...
    public static final String
        findPatientsWhoAreActiveOnArtAndInAtleastOneDSDWithAnyViralLoadValue =
            "         select patient_id from ( "
                ...
                + "		)maxConsulta group by maxConsulta.patient_id "
                + "	) tipodispensa "
                + "	inner join encounter e on (e.patient_id=tipodispensa.patient_id and tipodispensa.data_max_consulta=e.encounter_datetime) "
                + "	inner join obs o on o.encounter_id=e.encounter_id "
                + "     where e.voided=0 and e.encounter_type = 6 and o.concept_id=23739 and o.value_coded in(23720,23888) and o.voided=0 and e.location_id=:location "
                + "     union "
                + "	select patient_id from ( "
                + "	select carga_viral.patient_id, max(fila.data_levantamento) data_levantamento from  ( "
                + "			Select p.patient_id,max(o.obs_datetime) data_carga from patient p "
